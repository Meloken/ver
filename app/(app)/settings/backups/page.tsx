  ...
  return (
    <div className="container flex flex-col gap-4">
      <div className="flex flex-col gap-4">
        <h1 className="text-2xl font-bold">{t("downloadTitle")}</h1>
        <div className="flex flex-row gap-4">
          <Button onClick={handleDownload} disabled={isLoading || isDownloading}>
            {isLoading ? (
              ...
              <>
                <Download className="mr-2" /> {t("downloadButton")}
              </>
            )}
          </Button>
        </div>
      ...
      <Card className="flex flex-col gap-2 mt-16 p-5 bg-red-100 max-w-xl">
        <h2 className="text-xl font-semibold">{t("restoreTitle")}</h2>
        ...
        <form action={restoreBackup}>
          <div className="flex flex-col gap-4 pt-4">
            ...
            <label className="flex flex-row gap-2 items-center">
              ...
              <span className="text-red-500">{t("confirmDelete")}</span>
            </label>
            <Button type="submit" variant="destructive" disabled={restorePending}>
              {restorePending ? (
                <>
                    <Loader2 className="animate-spin" /> {t("restoring")}
                </>
              ) : (
                t("restoreButton")
              )}
      ...
      {restoreState?.success && (
        <Card className="flex flex-col gap-2 p-5 bg-green-100 max-w-xl">
          <h2 className="text-xl font-semibold">{t("restoreSuccess")}</h2>
          <p className="text-sm text-muted-foreground">{t("restoreSuccessDesc")}</p>
          <ul className="list-disc list-inside">
            {Object.entries(restoreState.data?.counters || {}).map(([key, value]) => (
              <li key={key}>
                <span className="font-bold">{key}</span>: {value} {t("items")}
              </li>
            ))}
          </ul>
        </Card>
